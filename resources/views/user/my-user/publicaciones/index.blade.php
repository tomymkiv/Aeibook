<x-layout>
    <div class="mt-20">
        <h1 class="text-4xl text-white/75">Publicaciones</h1>
    </div>
    ...
    @else
        ...
        {{-- @foreach ($users->post as $user->post o algo asi, que solo muestre las publicaciones del usuario seleccionado) --}}
            <x-muro.publicaciones>
                <x-slot:userPhoto> {{ Vite::asset('resources/images/pichu.png') }} </x-slot:userPhoto>
                <x-slot:username>usuario actual</x-slot:username>
                <x-slot:imgPosted>{{ Vite::asset('resources/images/pichu.png') }}</x-slot:imgPosted>
                <x-slot:descripcion>Descripcion: Lorem ipsum dolor sit amet consectetur adipisicing elit. Alias modi dolore iste exercitationem voluptatum officiis aperiam vel. Aliquam sit, eveniet numquam sunt, nulla quia quis voluptatum et pariatur officiis dolorem.</x-slot:descripcion>
            </x-muro.publicaciones>
        {{-- @endforeach --}}
        <div class="modal-container hidden relative z-10" aria-labelledby="modal-title" role="dialog" aria-modal="true">
            <!--
