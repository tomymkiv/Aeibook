    ...
    <div class="p-5 w-auto bg-white/90 rounded-lg md:max-w-[480px] md:min-w-[480px]">
        <div class="space-y-12">
            <div class="flex flex-row items-center gap-3">
                <a href="/user/{{ $userId }}/perfil"
                    class="w-20 h-20 flex items-center justify-center border-white/30 rounded-full aspect-square object-cover hover:shadow-md shadow-zinc-400 transition duration-300">
                    <x-rounded-img src="{{ asset($profilePhoto) }}" />
                </a>
                <a href="/user/{{ $userId }}/perfil"
                    class="hover:underline text-lg w-full font-semibold">{{ $username }}
                    @auth
            ...
            @if (!empty($postFileContent) && !$video)
                ...
                <div class="flex items-center justify-center">
                    <img src="{{ asset($file) }}" class="rounded-lg w-full max-h-100 object-contain" alt="Imagen de la publicacion"/>
                </div>
            @endif
